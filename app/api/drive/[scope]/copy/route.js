...
import { copyKeys } from '@/lib/r2/transfer';
...
export async function POST(req, { params }) {
  ...
  try {
    ...
    const res = await copyKeys({ bucket: scope.bucket, keys, destRel: destPrefix, scopeRoot: scope.rootPrefix });
    return NextResponse.json({ ok: true, copied: res });
  } catch (err) {
